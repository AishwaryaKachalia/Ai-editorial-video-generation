...
// Placeholder scene using flat colors instead of real images, so it renders
// with zero assets. Swap `color` for `src` (a file under /public) per tile.
export const exampleScenes: Scene[] = [
  {
    ...
    tiles: [
      ...
      {id: 's1-b', x: 65, y: 30, width: 45, height: 30, rotation: 3, startFrame: 6, duration: 18, color: '#c2a98f'},
  ...
  },
  {
    ...
    background: '#ece7de',
    tiles: [
      {id: 's2-a', x: 50, y: 40, width: 60, height: 40, rotation: -2, startFrame: 0, duration: 20, color: '#b5c4bb'},
